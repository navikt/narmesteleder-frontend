...
describe("withBackendMode", () => {
  const realFn = vi.fn((x: number) => x + 1);
  const fakeFn = vi.fn((x: number) => x - 1);
  ...
  it("calls real implementation when isLocalOrDemo is false", () => {
  ...
  it("calls fake implementation when isLocalOrDemo is true", () => {
